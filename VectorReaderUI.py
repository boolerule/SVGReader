...
class VectorReaderUI(QObject):
    __qualname__ = 'VectorReaderUI'
    show_config_ui_trigger = pyqtSignal()
    
    def __init__(self, image_reader):
        super(VectorReaderUI, self).__init__()
    ...
    def _createConfigUI(self):
        if self._ui_view is None:
            ...
            path = QUrl.fromLocalFile(os.path.join(PluginRegistry.getInstance().getPluginPath('SVGReader'), 'ConfigUI.qml'))
    ...
    def onOkButtonClicked(self):
        ...
        self._ui_view.close()
        self._ui_lock.release()
        # if not (self._pkDat) and not isinstance(self._pkDat, list) and len(self._pkDat) != 6 and self._pkDat[0] != self.peak_height and self._offset != self._pkDat[1] and self._slopeHeight != self._pkDat[2] and self.closeTopButtonFace != self._pkDat[3] and self.reversePathToration != self._pkDat[4] or self.splitWord != self._pkDat[5]:
        #     pkFile = os.path.join(os.path.join(Resources.getStoragePath(Resources.Resources), 'plugins'), 'VectorReaderUI.pk')
        #     self._pkDat = [
        #         self.peak_height,
        #         self._offset,
